#This code is written by Dr. Brenton Walker. I worked with him as a research assistant. He is one of the best resercher i ever worked.
#I am really lucky to get a chance to work with such a humble and gentle research mind. 
...
def main():
    ...
    with f:
        for line in f:
            ...
            if evt['Event'] == 'SparkListenerJobStart':
                ...
                print("SparkListenerJobStart "+str(job_id))#have to commnet bks
                events[job_id] = {
                                  'job_id': job_id,
                ...
                #print("SparkListenerStageSubmitted")#have to commnet bks
                pass
    ...
    with open(args.outfile+".path", 'w') as path, open(args.outfile+".jobdat", 'w') as jobdat:
        for job_id in sorted(events.keys()):
            ...
            for stage_id in sorted(job['stages'].keys()):
                ...
                for task_id in sorted(stage['tasks'].keys()):
                    ...
                    #-------------------------------------------------------------------------------------------------
                    path.write("\t".join([str(job_id), str(stage_id), str(task_id), \
            ...
            #-------------------------------------------------------------------------------------------------
            jobdat.write("\t".join([str(job_id), str(job['sojourn_time']), str(job['waiting_time']), str(job['sojourn_time'] - job['waiting_time'])])+"\n")
            mean_n += 1
    ...
    if args.distfile:
        distributions = {}
        for i in range(0, (max_time_interval//bin_width)+1):
            distributions[i] = {
        ...
        with open(args.outfile+".dist", 'w') as dist:
            ...
            scheduler_sum = 0.0
            run_time_sum = 0.0
            for i in range(0, (max_time_interval//bin_width)+1): #changed / to // becase of float and int prob and xrange to range for version prob
                ...
                #-------------------------------------------------------------------------------------------------
                dist.write("\t".join([str(distributions[i]['dt']),
